...
public final class ServiceGenerator {
    ...
    private func generateServerServiceImplementation(service s: TService, printer p: inout CodePrinter) throws {
        ...
        for method in methods {
            ...
            p.indent()
            p.print("switch result {\n")
            p.print("case .success(let value):\n")
            p.indent()
            p.print("do {\n")
            p.indent()
            p.print("completion.c(.success(try JSONEncoder().encode(value)))\n")
            p.outdent()
            p.print("} catch {\n")
            p.indent()
            p.print("completion.c(.failure(RTError(code: .encodeError, errorDescription: error.localizedDescription)))\n")
            p.outdent()
            p.print("}\n")
            p.outdent()
            p.print("case .failure(let error):\n")
            p.indent()
            p.print("completion.c(.failure(error))\n")
            p.outdent()
            p.print("}\n")
            p.outdent()
            p.print("}\n")
