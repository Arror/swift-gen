...
class ServiceGenerator {
    ...
    func generateThriftServices(printer p: inout CodePrinter) {
        for s in self.service {
            self.generateService(s: s, printer: &p)
        }
    }
    
    func generateThriftServiceRequests(printer p: inout CodePrinter) {
        for s in self.service {
            self.generateServiceRequest(s: s, printer: &p)
        }
    }
    
    private func generateService(s: TService, printer p: inout CodePrinter) {
        ...
        p.indent()
        for method in methods {
            p.print("@objc func __\(method.name)(parameters: Data, completion: @escaping (Data) -> Void) {\n")
            p.indent()
            p.print("let req = \(s.name)\(method.generateParameterStructName()).__rt_from(data: parameters)\n")
    ...
    private func generateServiceRequest(s: TService, printer p: inout CodePrinter) {
        let methods = s.methods.map { $0.value }
        p.print("\n")
        ...
        for method in methods {
            ...
            p.print("\n")
            p.print("public struct \(method.generateParameterStructName()): Codable {\n")
            p.indent()
            ...
            p.print("public static func \(method.name)(\(arguments)) throws -> RTRequest<\(s.name)\(method.generateParameterStructName()), String> {\n")
            p.indent()
            p.print("return try RTRequest(\n")
            p.indent()
            p.print("methd: \"\(s.name).\(method.name)\",\n")
            p.print("parameter: \(s.name).\(method.name)(\(method.arguments.map({ "\($0.name): \($0.name)" }).joined(separator: ", "))),\n")
